...
def _dflash_layer_causal(config: Qwen3Config, layer_idx: int) -> bool:
    """Resolve explicit causality (``config.is_causal``, then
    ``dflash_config.causal``) before falling back to legacy layer
    defaults: only SWA layers of a *mixed* ``layer_types`` are causal."""
