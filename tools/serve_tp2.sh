...
#
# Both B70s in one container, exactly as tp2run.sh / serve_pp2.sh do it:
# separate processes, separate Level Zero contexts, and ONLY the two B70
# render nodes mounted, so neither rank can land on the B580 or the iGPU.
...
echo "launched $CNAME ${CID:0:12} on port $PORT, tensor parallel over $NODE0 + $NODE1"
