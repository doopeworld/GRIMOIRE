...
static int run_rank(const std::string& dir, const std::string& out, Fmt fmt) {
    ...
    for (const auto& p : requests()) {
        ...
        FinishReason r{};
        try {
            if (!grimoire_pp_broadcast_request(*e, p, kWant, -1, -1)) {
                std::fprintf(stderr, "broadcast failed\n");
                grimoire_delete(e); return 6;
            }
            grimoire_serve_generate(*e, p, kWant, -1, toks, -1, {}, &r);
...
int main(int argc, char** argv) {
    ...
    std::printf("=== a resident pipeline, serving ===\n");
