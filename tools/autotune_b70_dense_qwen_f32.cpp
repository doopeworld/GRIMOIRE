...
static double run(sycl::queue&q,int p,const Shape&z,std::vector<float>&ref,
 ...
 auto*y=sycl::malloc_device<float>(size_t(M)*z.n,q);
 q.fill(x,bf16(.125f),size_t(M)*z.k);q.memset(w,0x22,size_t(z.n)*z.k/2);
 q.memset(s,127,size_t(z.n)*z.k/32);q.wait();
 auto launch=[&]{grimoire_xe2_dense_mxfp4_f32_autotune(
  &q,x,w,s,y,M,z.n,z.k,p);q.wait_and_throw();};
 ...
 size_t ns=std::min<size_t>(4096,size_t(M)*z.n);std::vector<float>o(ns);
 q.memcpy(o.data(),y,ns*sizeof(float)).wait();if(ref.empty())ref=o;err=0;
 for(size_t i=0;i<ns;++i)err=std::max(err,std::abs(double(o[i]-ref[i])));
 sycl::free(x,q);sycl::free(w,q);sycl::free(s,q);sycl::free(y,q);return best;}
int main(){sycl::queue q{sycl::gpu_selector_v,
 {sycl::property::queue::in_order{}}};
 const char*pn[]={"32x64","32x128","32x256","64x64","64x128",
  "64x256","128x64","128x128","128x256"};
 constexpr size_t cb=size_t(768)<<20;
 auto*cold=sycl::malloc_device<unsigned char>(cb,q);
 std::vector<float>refs[8];double totals[9]={};bool ok[9];
 std::fill(ok,ok+9,true);
 for(int j=0;j<8;++j){double e;run(q,7,shapes[j],refs[j],e,cold,cb);}
 std::puts("policy,shape,N,K,freq,cold_ms,weighted_ms,max_abs,status");
 for(int p=0;p<9;++p)for(int j=0;j<8;++j){try{double e;
  double ms=run(q,p,shapes[j],refs[j],e,cold,cb);bool pass=e<=.01;
  ok[p]&=pass;totals[p]+=ms*shapes[j].freq;
  std::printf("%s,%s,%d,%d,%d,%.3f,%.3f,%.6g,%s\n",pn[p],
   shapes[j].name,shapes[j].n,shapes[j].k,shapes[j].freq,ms,
   ms*shapes[j].freq,e,pass?"PASS":"REJECT_NUMERIC");
 }catch(const std::exception&e){ok[p]=false;
  std::printf("%s,%s,%d,%d,%d,nan,nan,nan,REJECT:%s\n",pn[p],
   shapes[j].name,shapes[j].n,shapes[j].k,shapes[j].freq,e.what());}}
 int win=-1;double best=INFINITY;for(int p=0;p<9;++p){
  std::printf("TOTAL,%s,%.3f,%s\n",pn[p],totals[p],ok[p]?"PASS":"REJECT");
  if(ok[p]&&totals[p]<best){best=totals[p];win=p;}}
 if(win<0)return 2;std::printf(
  "WINNER policy=%s weighted_total=%.3f ms candidates=72\n",pn[win],best);
