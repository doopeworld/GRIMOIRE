...
bool Qwen35Model::load(const std::string& d, std::string& err, bool skip_vision,
    ...
    // ---- config -----------------------------------------------------
    cfg.hidden        = cfg_i(j, "hidden_size", 0);
    cfg.n_layers      = cfg_i(j, "num_hidden_layers", 0);
    cfg.vocab         = cfg_i(j, "vocab_size", 0);
    cfg.n_heads       = cfg_i(j, "num_attention_heads", 0);
    cfg.n_kv_heads    = cfg_i(j, "num_key_value_heads", 0);
    cfg.head_dim      = cfg_i(j, "head_dim", 0);
    cfg.rms_eps       = cfg_f(j, "rms_norm_eps", 1e-6f);
    cfg.rope_theta    = cfg_f(j, "rope_theta", 1e7f);
    ...
    cfg.dense_inter   = cfg_i(j, "intermediate_size", 0);
    ...
    if (::access(native_path.c_str(),R_OK)==0) {
        ...
        for(uint64_t i=0;i<native_model->header().tensor_count;++i){
            ...
            if(skip_vision&&name.find(".visual.")!=std::string::npos)continue;
        ...
        for (size_t i = 0; i < hf.shards.size(); ++i) {
            ...
            for (const auto& kv : st->tensors()) {
                if (skip_vision && kv.first.find(".visual.") != std::string::npos) continue;
                TensorRef r;
                r.shard = int(i);
                r.t     = kv.second;
                index[kv.first] = r;
