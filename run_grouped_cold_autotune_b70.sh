...
COMMON=(
  ...
  -include /src/csrc/sycl_first.h -DVLLM_XPU_ENABLE_XE2
  -DGRIMOIRE_ENABLE_AUTOTUNE
  -DCUTLASS_ENABLE_HEADERS_ONLY -DCUTLASS_ENABLE_SYCL
  -DSYCL_INTEL_TARGET -DCUTLASS_VERSIONS_GENERATED
  -Isrc -I/src -I/src/csrc -I/src/csrc/xpu/grouped_gemm/xe_2
...
icpx "${COMMON[@]}" src/xe2_grouped_bridge.cpp \
  -o /tmp/libgrimoire_grouped_cold_tune.so -lze_loader
icpx -O3 -std=c++17 -fsycl tools/autotune_b70_grouped.cpp \
  /tmp/libgrimoire_grouped_cold_tune.so -Wl,-rpath,/tmp \
  -o /tmp/autotune_b70_grouped_cold
/tmp/autotune_b70_grouped_cold
