...
#include "csrc/xpu/gdn_attn/xe_2/gemm.hpp"
#include "csrc/xpu/gdn_attn/gdn_attn_utils.h"
...
#ifndef GRIMOIRE_GDN_DV_PARTS
#define GRIMOIRE_GDN_DV_PARTS 2
#endif
#ifndef GRIMOIRE_GDN_FWD_SM
#define GRIMOIRE_GDN_FWD_SM 4
#endif
#ifndef GRIMOIRE_GDN_FWD_SN
#define GRIMOIRE_GDN_FWD_SN 2
#endif
struct chunk_gemm_policy_fwd_tuned {
  using WGTile = Shape<_64, _64, _32>;
  using SGLayout = Layout<
      Shape<Int<GRIMOIRE_GDN_FWD_SM>,Int<GRIMOIRE_GDN_FWD_SN>,_1>,
      Stride<Int<GRIMOIRE_GDN_FWD_SN>,_1,_0>>;
};
...
using chunk_gemm_policy_fwd_o = chunk_gemm_policy_fwd_tuned;
...
CUTE_DEVICE void chunk_compute_wu_kernel(
  ...
  int group_id = item.get_group(1);
  int group_range = item.get_group_range(1);
  // Spread work across (value-head, chunk) instead of assigning one
  // workgroup to a chunk and serially processing all value heads.
  const int global_chunk_range = group_range / num_v_heads;
  int chunk_id = group_id % global_chunk_range;
  const int v_head_id = group_id / global_chunk_range;
  if (v_head_id >= num_v_heads) return;
  ...
  for (int batch_id = 0; batch_id < batch_size; ++batch_id) {
    ...
    while (chunk_id < cumsum_chunks) {
      ...
      // WAR fence: previous iteration's sub-groups may still be reading
      // beta_slm_ptr/g_slm_ptr; all must arrive before the next refill.
      item.barrier(sycl::access::fence_space::local_space);
        CUTE_UNROLL
        for (int e = local_id; e < chunk_size; e += local_range) {
          float beta_value =
...
CUTE_DEVICE void chunk_fwd_o_kernel(
  ...
  constexpr int dv_parts = GRIMOIRE_GDN_DV_PARTS;
  const int v_group = item.get_group(1);
  int v_head_id = v_group / dv_parts;
  const int dv_part = v_group % dv_parts;
  int local_range = item.get_local_range(2);
  ...
  for (int batch_id = 0; batch_id < batch_size; ++batch_id) {
    ...
    for (int chunk_id = 0; chunk_id < current_chunks; ++chunk_id) {
      ...
      if (has_prev_state) {
        ...
        for (int dv = dv_part; dv < head_v_dim / chunk_size; dv += dv_parts) {
...
void kernel_launcher(
  ...
  auto ev_prepare = queue.submit([&](sycl::handler& cgh) {
  ...
  auto ev_compute_a = queue.submit([&](sycl::handler& cgh) {
  ...
  sycl::event ev_inverse;
  // This bridge is compiled specifically for intel_gpu_bmg_g31.  Avoid the
  // upstream runtime architecture probe, which reaches through c10/XPU and
  // would make the otherwise raw shared library depend on Torch symbols.
  if (true) {
    ...
    ev_inverse = queue.submit([&](sycl::handler& cgh) {
  ...
  auto ev_compute_wu = queue.submit([&](sycl::handler& cgh) {
    sycl::local_accessor<float, 1> local_mem(
  ...
  sycl::range<3> global_fwd_o(batch_size,
      num_v_heads * GRIMOIRE_GDN_DV_PARTS, 1);
  int slm_size_fwd_o = chunk_size + chunk_size + chunk_size;

  auto ev_fwd_o = queue.submit([&](sycl::handler& cgh) {
    sycl::local_accessor<float, 1> local_mem(
  ...
  if (std::getenv("GRIMOIRE_GDN_PROFILE")) {
    ev_fwd_o.wait();
    auto ms=[](const sycl::event& e){
      auto a=e.get_profiling_info<sycl::info::event_profiling::command_start>();
      auto b=e.get_profiling_info<sycl::info::event_profiling::command_end>();
      return double(b-a)*1e-6;
    };
    std::fprintf(stderr,"GDN passes prepare %.3f A %.3f inverse %.3f WU %.3f fwd %.3f ms\n",
        ms(ev_prepare),ms(ev_compute_a),ms(ev_inverse),ms(ev_compute_wu),ms(ev_fwd_o));
  }
}

// The upstream file also carries a torch::Tensor adapter below.  Grimoire's
// raw bridge calls kernel_launcher directly, so keep that adapter out of this
// standalone build rather than introducing a Torch runtime dependency.
#if defined(GRIMOIRE_GDN_ENABLE_TORCH_ADAPTER)
#include <torch/all.h>
...
#endif  // GRIMOIRE_GDN_ENABLE_TORCH_ADAPTER
